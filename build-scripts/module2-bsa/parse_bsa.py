...
raw = open('bsa_full.txt', encoding='utf-8').read()
lines = raw.split('\n')
...
while i < len(arr_lines):
    ...
    if not m or int(m.group(1)) != prev + 1:
        i += 1
        continue
    n = int(m.group(1))
    prev = n
    ...
    while j < len(arr_lines):
        ...
        if mm:
            if int(mm.group(1)) == prev + 1:
                break                       # real next section
            buf.append(mm.group(1) + '.')   # wrapped cross-reference -> keep in heading
            j += 1
            continue
        if s.startswith('Note:') or 'For Reference only' in s or len(' '.join(buf)) > 240:
            break
        is_divider = (s.startswith('CHAPTER') or s.startswith('PART ')
        ...
        buf.append(s)
        j += 1
    h = re.sub(r'\s+', ' ', ' '.join(buf)).strip().rstrip('.').strip()
    headings[n] = h
    i = j
...
def anchor(n):
    return re.compile(r'(?m)^%d\.(?=\s|\(|[A-Za-z])' % n)
...
json.dump(sections, open('bsa_sections.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('body parsed:', len(sections), 'sections written to bsa_sections.json')

# quick sanity dump
for k in (1,2,3,24,25,26,27,39,58,61,63,105,120,143,170):
    if k in sections:
        s = sections[k]
        print('\n----', k, '|', s['heading'], '----')
        print(s['text'][:700])
